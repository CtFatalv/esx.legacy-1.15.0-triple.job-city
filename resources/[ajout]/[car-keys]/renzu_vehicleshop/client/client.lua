    ...
    PopulateVehicleshop = function(k)
        ...
        for _, value in pairs(tb) do
            ...
            if IsModelInCdimage(vehicleModel) then
                ...
                local categoryName = value.category or "Autres"
                
                if not Vehicles[categoryName] then 
                    Vehicles[categoryName] = {} 
                end

                -- On stocke la catégorie pour les onglets NUI
                brands[categoryName] = value.shop

                local vehname = value.name or GetDisplayNameFromVehicleModel(value.model)
                local pmult, tmult = 1000, 800
                local stats = GetPerformanceStats(vehicleModel)
                local handling, brake = stats.handling, stats.brakes
                
                if shoptype == 'boat' or shoptype == 'plane' then
                    pmult, tmult, handling, brake = 10, 8, handling * 0.1, brake * 0.1
                end
                ...
                local VTable = {
                    brand = categoryName, -- Affiché comme titre dans le NUI
                    name = vehname:upper(),
                    brake = brake,
                    handling = handling,
                ...
                table.insert(Vehicles[categoryName], VTable)
            end
        end

        SendNUIMessage({ container = "shop" })
        Wait(500)
        SendNUIMessage({
            brands = brands,
            type = "brands",
