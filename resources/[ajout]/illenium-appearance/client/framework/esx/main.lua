...
RegisterNetEvent("esx:playerLoaded", function(xPlayer, isNew, skin)
    ...
    -- Correctif : Re-téléporte le joueur UNIQUEMENT si ce n'est PAS un nouveau personnage
    if not isNew and xPlayer and xPlayer.coords then
        CreateThread(function()
            Wait(500) -- Attente pendant que InitAppearance réinstancie le Ped
            local ped = PlayerPedId()
            SetEntityCoords(ped, xPlayer.coords.x, xPlayer.coords.y, xPlayer.coords.z -1, false, false, false, false)
            SetEntityHeading(ped, xPlayer.coords.heading or 0.0)
        end)
    end
end)
