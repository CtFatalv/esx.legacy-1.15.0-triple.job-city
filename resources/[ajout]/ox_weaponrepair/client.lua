    ...
    if items then
        for name, data in pairs(items) do
            for _, v in pairs(data) do
                if v.slot then
                    options[#options+1] = {id = name .. v.slot, title = v.label, description = string.format('Durabilité: %s', v.metadata.durability .. '%'), serverEvent = 'OT_weaponrepair:startweaponrepair', args = {slot = v.slot, name = name, bench = i}, metadata = {{label = 'Requis', value = ''..Config.requireditemLabel..''}, {label = 'Montant', value = ''..Config.requireditemamount..''}}}
                end
            end
        end
        lib.registerContext({id = 'repairbench', title = 'Etabli', options = options})
...
for i = 1, #Config.locations do
    ...
    if target then
        ...
        function bench:onEnter()
            lib.showTextUI('[E] - Utiliser l\'établi', {icon = 'wrench'})
        end
