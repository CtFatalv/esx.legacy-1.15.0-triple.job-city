(() => {
  ...
  const transactionLabels = {
    ...
    WITHDRAW: "Retrait",
    TRANSFER: "Transfert",
    TRANSFER_RECEIVE: "Transfert reçu",
    PINCODE: "Code PIN modifier"
  };
  ...
  function renderActionForm() {
    if (state.activeAction === "transfer") {
      return `
        <form class="form-stack" data-action-form>
          <div class="form-row">
            <input name="amount" type="number" min="1" inputmode="numeric" placeholder="Montant" autocomplete="off" />
            <input name="target" type="number" min="1" inputmode="numeric" placeholder="Civil ID" autocomplete="off" />
          </div>
          ${renderQuickAmounts()}
    ...
    if (state.activeAction === "pincode") {
      return `
        <form class="form-stack" data-action-form>
          <input name="pin" type="password" maxlength="4" inputmode="numeric" placeholder="Nouveau code à 4 chiffres" autocomplete="off" />
          <button class="primary-button" type="submit" ${state.busy ? "disabled" : ""}>${state.busy ? "Processing" : "Enregistrer le Code PIN"}</button>
          ${renderFormError()}
        </form>
      `;
    }
    ...
    return `
      <form class="form-stack" data-action-form>
        <input name="amount" type="number" min="1" inputmode="numeric" placeholder="${state.activeAction === "deposit" ? "Espèces à déposer" : "Argent à retirer"}" autocomplete="off" />
        ${renderQuickAmounts()}
        <button class="primary-button" type="submit" ${state.busy ? "disabled" : ""}>${state.busy ? "En cours" : escapeHtml(actionLabels[state.activeAction])}</button>
        ${renderFormError()}
      </form>
    `;
  }
  ...
  function getActionContextRows() {
    if (state.activeAction === "withdraw") {
      return [
        { label: "Source", title: "Banque", value: formatMoney(state.data.bank) },
        { label: "Destination", title: "Espèces", value: formatMoney(state.data.cash) },
      ];
    }
    ...
    if (state.activeAction === "transfer") {
      return [
        { label: "Source", title: "Banque", value: formatMoney(state.data.bank) },
        { label: "Destination", title: "Civil", value: "Civil ID" },
      ];
    }

    if (state.activeAction === "pincode") {
      return [
        { label: "Securitée", title: "PIN", value: state.data.hasPin ? "Configuré" : "Non Configuré" },
        { label: "Accès", title: state.data.accessType === "atm" ? "ATM" : "Autorisé", value: "Active" },
      ];
    }

    return [
      { label: "Source", title: "Espèces", value: formatMoney(state.data.cash) },
      { label: "Destination", title: "Banque", value: formatMoney(state.data.bank) },
    ];
  }
  ...
  function renderActionContext(stats) {
    ...
    const latestLabel = latest ? transactionLabels[latest.type] || latest.label || "Mouvement" : "Dernier mouvement";
    ...
    if (!transactions.length) {
      return `
        <div class="flow-header">
          ...
          <span class="flow-pill">0 mouvement</span>
        </div>
        <div class="empty-state">Pas encore de mouvement</div>
      `;
    }
  ...
  function renderHistoryItems() {
    ...
    if (!transactions.length) {
      return '<div class="empty-state">Aucune transaction trouvé</div>';
    }
    ...
    return `
      <div class="pin-panel">
        ...
        <div class="pin-content">
          <div class="pin-title">
            <h2>${hasPin ? "Enter le Code PIN" : "PIN Obligatoire"}</h2>
            <p>${hasPin ? "Déverrouillez cette session ATM." : "Définissez un code PIN en agence bancaire avant d'utiliser les ATM.."}</p>
          </div>
          ${
            hasPin
              ? `
                ...
                <div class="pin-grid">
                  ${[1, 2, 3, 4, 5, 6, 7, 8, 9]
                    ...
                    .join("")}
                  <button class="pin-key danger" type="button" data-pin-clear>C</button>
  ...
  function renderBanking() {
    ...
    const accessLabel = state.data.accessType === "atm" ? "ATM session" : "Compte Client";
    const pinLabel = state.data.hasPin ? "Configuré" : "Non Configuré";
        ...
        <div class="bank-layout">
          <section class="main-panel">
            <div class="balance-strip">
              ...
              <article class="balance-card">
                ...
                <span class="sub-value">Argent en espèces</span>
              </article>
              <article class="balance-card">
                <span class="label">Total</span>
                <strong class="value">${formatMoney(total)}</strong>
                <span class="sub-value">Espèces + banque</span>
              </article>
            </div>
            ...
            <section class="history-panel">
              <div class="section-header">
                <h2>Activitée Récente</h2>
                <input class="history-search" type="text" value="${escapeHtml(state.search)}" placeholder="Recherche de transaction..." data-history-search />
              </div>
          ...
          <aside class="side-panel">
            <section class="bank-card">
              ...
              <div class="card-label">Carte de débit</div>
              <div class="card-number">5464 8794 1213 9871</div>
  ...
  if (isBrowser()) {
    openBanking({
      accessType: "banque",
      bankName: "Fleeca Banque",
      playerName: "Transaction CLient",
      cash: 12850,
      bank: 76400,
      hasPin: true,
      transactions: [
        { label: "Paye", type: "DEPOSIT", amount: 2500, time: Date.now() - 900000, balance: 76400 },
        { label: "Vehicle répairé", type: "WITHDRAW", amount: 650, time: Date.now() - 3400000, balance: 73900 },
        { label: "Transfert", type: "TRANSFER", amount: 1200, time: Date.now() - 8600000, balance: 74550 },
        { label: "Transfert reçu", type: "TRANSFER_RECEIVE", amount: 4200, time: Date.now() - 14800000, balance: 75750 },
        { label: "Achat en magasin", type: "WITHDRAW", amount: 180, time: Date.now() - 24000000, balance: 71550 }
      ]
    });
  }
})();
